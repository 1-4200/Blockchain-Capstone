...
contract Pausable is Ownable {
    ...
    modifier whenPaused() {
        require(_paused == true, "should be paused");
        _;
    }

    modifier whenNotPaused() {
        require(_paused == false, "should not be paused");
        _;
    }

    event Pause(address by);
    event Unpause(address by);
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        require(to != ownerOf(tokenId), "sent address owns sent token");
        require(msg.sender == owner() || isApprovedForAll(owner(), msg.sender), "sender should be owner of contract");
        _tokenApprovals[tokenId] = msg.sender;
        emit Approval(owner(), to, tokenId);
    }

    function getApproved(uint256 tokenId) public view returns (address) {
        require(_exists(tokenId), "token should be existed");
...
contract CustomERC721Token is ERC721Metadata {
    constructor (string memory _name, string memory _symbol) ERC721Metadata(_name, _symbol, "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") public {
    }
